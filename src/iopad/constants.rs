//! # I/O Pad Constants
//!
//! This module defines constants and enumerations for I/O pad configuration including:
//! - Function multiplexing options
//! - Pull resistor configurations
//! - Drive strength settings
//! - Delay tuning options
//! - Register offsets for various pads

/// I/O pad function selection.
///
/// Each pad can be configured to one of 8 different functions.
...
/// I/O pad drive strength setting.
///
/// Each pad can be configured with one of 16 different drive strength levels.
#[derive(Debug, Clone, Copy)]
pub enum FioPadDrive {
    ...
    /// Number of drive strength options
    NumOfDrive,
}
...
pub enum FioPadDelayDir {
    /// Delay setting for output signals
    OutputDelay = 0,
    /// Delay setting for input signals
    InputDelay,

    /// Number of delay direction options
    NumOfDelayDir,
}
...
pub enum FioPadDelayType {
    /// Coarse delay tuning (larger steps)
    DelayCoarseTuning = 0,
    /// Fine delay tuning (smaller steps)
    DelayFineTuning,

    /// Number of delay type options
    NumOfDelayType,
}

/// I/O pad delay value.
///
/// Delay values range from 0 (no delay) to 7 (maximum delay).
#[derive(Debug, Clone, Copy)]
...
/// Register 0 offset for AN59 pad
